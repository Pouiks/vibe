...
export function InstallPrompt() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 p-4 animate-slide-up pb-8">
      <div className="bg-white p-5 rounded-3xl relative overflow-hidden shadow-lg border border-slate-200">
        
        <button 
          onClick={handleDismiss}
          className="absolute top-3 right-3 text-slate-400 hover:text-slate-600 p-1 rounded-full bg-slate-100"
        >
          <X className="w-4 h-4" />
        </button>

        <div className="flex items-start gap-4">
          <div className="bg-blue-600 rounded-2xl p-3 shadow-lg shadow-blue-600/20 shrink-0">
            <Download className="w-6 h-6 text-white" />
          </div>
          
          <div className="pt-0.5">
            <h3 className="text-slate-900 font-bold text-sm mb-1">Installer VibeSpot</h3>
            <p className="text-xs text-slate-500 leading-relaxed mb-3">
              Ajoute l'app sur ton écran d'accueil pour l'ouvrir en fond d'écran !
            </p>
            
            {isChromeIOS ? (
              <div className="bg-orange-50 rounded-xl p-3 text-[11px] font-medium text-orange-700 border border-orange-200">
              ...
              <div className="bg-slate-50 rounded-xl p-3 text-[11px] font-medium text-slate-600 border border-slate-200">
